...
```{r}
if (!file.exists("cookbooks.txt")) prep_word2vec(origin="cookbooks",destination="cookbooks.txt",lowercase=T,bundle_ngrams=3)
```
...
```{r}
if (!file.exists("cookbook_vectors_3.bin")) {
  model = train_word2vec("cookbooks.txt","cookbook_vectors_3.bin",vectors=200,threads=4,window=12,iter=5,negative_samples=0)
  } else model = read.vectors("cookbook_vectors_3.bin")
...
model %>% closest_to("overcooked")
model %>% closest_to("raw")
```
...
```{r}
model %>% 
  closest_to(model[[c("liver","salmon","trout","shad","flounder","carp","roe","eels")]],50)
```
...
```{r}
some_fish = closest_to(model,model[[c("fish","salmon","trout","shad","flounder","carp","roe","eels")]],25)
...
term_set = lapply(ingredients, 
       function(ingredient) {
          nearest_words = model %>% closest_to(model[[ingredient]],15)
          nearest_words$word
        }) %>% unlist
...
sweet_and_saltiness = model[1:30,] %>% cosineSimilarity(tastes)
