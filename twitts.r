...
#getting tweets
tweets.df<-searchTwitter("#mining +#silver",n=100,lang = "en")%>%twListToDF()
glimpse(tweets.df)

write.csv(tweets.df,file="Twitter_mining_feed.csv")
### Trend locationsa
...
#
#
s.bdy      <- get_nrc_sentiment(corpus.bdy)%>%colSums(na.rm=TRUE)
#s.frm      <- get_nrc_sentiment(corpus.frm)%>%colSums(na.rm=TRUE)%>%sort
s.sub      <- get_nrc_sentiment(corpus.sub)%>%colSums(na.rm=TRUE)
#
barplot(s.bdy,las=2,col=rainbow(50),ylab="count",main="sentiment for PACE mail BODY")
